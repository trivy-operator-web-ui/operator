...
use trivy_operator_web_ui::{
    api::dto::SimpleSbomReportDTO,
    kube_types::{Artifact, SbomReport},
};
use zip::ZipArchive;

mod utils;
use utils::{
    ETCD, RABBIT_ONE, SBOM_REPORT_GVK, apply_test_resource, cleanup_test_namespace,
...
async fn zip_sbom_reports() -> Result<()> {
    ...
    let mut zip = http_client
        ...
        .cookie(Cookie::new("jwtToken", &jwt))
        .send_json(&vec![
            etcd.report.artifact.clone(),
            rabbit_one.report.artifact.clone(),
        ])
        .await
        .unwrap();

    assert!(zip.status() == StatusCode::OK);

    let zip = zip.body().await?;

    let zip = ZipArchive::new(Cursor::new(zip)).unwrap();
    assert!(zip.len() == 2);
