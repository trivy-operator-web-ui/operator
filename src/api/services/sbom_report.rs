...
use crate::{api::{dto::SimpleSbomReportDTO, error::ZipSbomError}, kube_types::{Artifact, sbom_report::ImageSbomReport}, states::ReportState};
...
impl SbomReportService {
    ...
    pub fn zip_sboms_by_artifacts(&self, artifacts: Vec<Artifact>) -> Result<Vec<u8>, ZipSbomError> {
        let sbom_reports = self.state.reports.lock().unwrap().clone();

        let sbom_reports_artifacts: HashSet<Artifact> = HashSet::from_iter(sbom_reports.keys().cloned());
...
mod tests {
    ...
    use crate::{api::{error::ZipSbomError, services::SbomReportService}, kube_types::{Artifact, SbomReport, Workload, sbom_report::ImageSbomReport}, states::ReportState};
    ...
    fn get_simple_sbom_reports() -> Result<()> {
        ...
        let simple_sbom_report = 
            sbom_report_state.get_simple_sbom_reports();
    ...
    fn zip_sbom_reports_with_known_artifacts() -> Result<()> {
        ...
        let sbom_report_zip = 
            sbom_report_state.zip_sboms_by_artifacts(existing_artifacts);
    ...
    fn zip_sbom_reports_with_unknown_artifacts() -> Result<()> {
        ...
        let sbom_report_zip = 
            sbom_report_state.zip_sboms_by_artifacts(unexisting_artifacts);

        assert!(sbom_report_zip.is_err_and(|err| err == ZipSbomError::ArtifactsNotFound(HashSet::from([dummy_artifact]))));
