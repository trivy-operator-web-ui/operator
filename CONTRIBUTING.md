...
- **"Integration Tests"** that will perform API calls **against an instance of this application deployed in a Kubernetes Cluster**. This time you'll need a real Kubernetes Cluster with a working deployment of this application. The easiest way is to deploy it using its [Helm Chart](https://github.com/trivy-operator-web-ui/helm-chart).

**Common tests helpers** to iniate services, states etc.. with the test ressources are avaiable :
- In the [common_test_helpers module](./src/common_test_utils/mod.rs) and the [services' helpers file](./src/api/services/tests_utils.rs) for unit tests.
- In the [integration test utils module](./tests/utils/mod.rs) for integration tests.
...
To run the tests, first install [nextest](https://nexte.st/). Then :
- For the unit tests, use the `cargo nextest run --no-fail-fast --lib -j1` command.
- For the integration tests, use the `cargo nextest run --no-fail-fast --test '*' -j1` command.

## 5. Bruno

To get familiar with the API, a [Bruno](https://www.usebruno.com/) project is available under the [.bruno](./.bruno/opencollection.yml) folder. Every addtion/change to the API **MUST** also be reflected in those files.
